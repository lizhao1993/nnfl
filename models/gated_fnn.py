...
class FNN(object):
    ...
    def __forward_propagation(self, x):
        ...
        # The output of gates
        ogates = tools.sigmoid_array(np.dot(vectorized_x, self.wig))
        ...
        return [py, hout, vectorized_x, ogates, vectorized_x_bak]
    ...
    def calculate_gradients(self, x, y):
        ...
        gigates = np.copy(ogates)
        for igate_sample, gogate_sample in zip(gigates, gogates):
            for i in range(0, igate_sample.shape[0]):
                wd = self.word2vec.shape[1]
                gogate_sum = gogate_sample[i*wd:(i+1)*wd].sum()
                igate_sample[i] = igate_sample[i] * (1 - igate_sample[i]) \
                                    * gogate_sum
