...
Authors: fengyukun
Date:   2016/4/20
Brief:  The implementation of Feedward Neural Network (FNN) with gates
"""
...
import sys
import operator
from scipy.stats import logistic    # not used
...
class FNN(object):
    ...
    def __init__(self, x, label_y, n_h, word2vec, lambda1=0.5, \
        ...
        # Init weights between input and gates
        self.wig = np.random.uniform(
            low=-np.sqrt(1. / self.n_i), 
            high=np.sqrt(1. / self.n_i), 
            size=(self.n_i, self.x.shape[1])
        ).astype(dtype=self.tfloat, copy=False)

        # Parameters of FNN
        self.params = [self.who, self.bo, self.wih, self.bh, self.wio, self.wig]
        self.params_names = ['who', 'bo', 'wih', 'bh', 'wio', 'wig']
    ...
    def __forward_propagation(self, x):
        ...
        vectorized_x_bak = np.copy(vectorized_x)
        # The output of gates
        igates = tools.sigmoid_array(np.dot(vectorized_x, self.wig))
        ogates = igates / igates.sum(axis=1).reshape((igates.shape[0], 1))
        for ogate_sample, vectorized_sample in zip(ogates, vectorized_x):
            for i in range(0, ogate_sample.shape[0]):
                wd = self.word2vec.shape[1]
                vectorized_sample[i*wd:(i+1)*wd] *= ogate_sample[i]
        # hidden net input
        ...
        return [py, hout, vectorized_x, ogates, vectorized_x_bak, igates]
    ...
    def predict(self, x):
        ...
        """
        try:
            fp_res = self.__forward_propagation(x)
            py = fp_res[0]
            ogates = fp_res[3]
        except:
        ...
        y = py.argmax(axis=1)
        return (np.array([self.y_to_label[i] for i in y]), ogates)
    ...
    def minibatch_train(self, lr=0.1, minibatch=5, \
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            error = metrics.zero_one_loss(self.label_y, label_preds[0])
    ...
    def calculate_total_loss(self):
        ...
        try:
            fp_res = self.__forward_propagation(self.x)
            py = fp_res[0]
        except:
            logging.error("Failed to run forward propagation")
            raise Exception
    ...
    def calculate_gradients(self, x, y):
        ...
        try:
            ...
            py = fp_res[0]
            hout = fp_res[1]
            vectorized_x = fp_res[2]
            ogates = fp_res[3]
            vectorized_x_bak = fp_res[4]
            igates = fp_res[5]
        except:
            logging.error("Failed to run forward propagation")
            raise Exception
        ...
        gvectorized_x = np.dot(ghnet, self.wih.T) \
            + np.dot(self.lambda2 * gonet, self.wio.T)
        # Gradients on the input of gates
        # gogates before sum
        gogates = gvectorized_x * vectorized_x_bak
        gigates = np.zeros(igates.shape)
        for t in range(0, gogates.shape[0]):
            gigate_sample = gigates[t]
            gogate_sample = gogates[t]
            igate_sample = igates[t]
            # Compute gradents before normalizing
            for i in range(0, gigate_sample.shape[0]):
                wd = self.word2vec.shape[1]
                gigate_sample[i] = gogate_sample[i*wd:(i+1)*wd].sum()
            igate_sum = igate_sample.sum()
            sq_sum = igate_sum ** 2
            g_sum = (gigate_sample * igate_sample).sum()
            for i in range(0, gigate_sample.shape[0]):
                gigate_sample[i] = (gigate_sample[i] * igate_sum - g_sum) \
                                    / sq_sum
                # Sigmoid derivative
                gigate_sample[i] *= igate_sample[i] * (1 - igate_sample[i])
        ...
        # Gradients on the wig
        gwig = np.dot(vectorized_x_bak.T, gigates)

        # The order is same as self.params
        gparams = [gwho, gbo, gwih, gbh, gwio, gwig, None, None]
        if self.up_wordvec:
            gx = np.copy(gvectorized_x)
            for ogate_sample, gx_sample in zip(ogates, gx):
                for i in range(0, ogate_sample.shape[0]):
                    wd = self.word2vec.shape[1]
                    gx_sample[i*wd:(i+1)*wd] *= ogate_sample[i]
            gx += np.dot(gigates, self.wig.T)
    ...
    def gradient_check(self):
        ...
        epsilon = 1e-04
        check_params = ['who', 'bo', 'wih', 'bh', 'wio', 'wig']
        # check_params = ['wig']
...
def gated_fnn_test1():
    """
    Test gated fnn
    Seed note: 0(small), 1(big), 10(middle)
    """
    n_h = 30
    vocab_size = 200
    word_dim = 2 
    ...
    preds_res = fnn.predict(x)
    for gates, x_i in zip(preds_res[1], x):
        for gate, x_ij in zip(gates, x_i):
            print("(%s:%s)\t" % (x_ij, gate), end='')
        print('')

    # return
    # Test
    print("\n\nDoing test\n\n")
    preds_res = fnn.predict(test_x)
    for gates, x_i in zip(preds_res[1], test_x):
        for gate, x_ij in zip(gates, x_i):
            print("(%s:%s)\t" % (x_ij, gate), end='')
        print('')
